...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $search = DataType::where('slug', 'search')->firstOrFail();


        $dataRow = $this->dataRow($search, 'id');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'number',
                'display_name' => __('id'),
        ...
        $dataRow = $this->dataRow($search, 'name');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('Наименование'),
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($search, 'count');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('количество'),
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($search, 'created_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
        ...
        $dataRow = $this->dataRow($search, 'updated_at');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
